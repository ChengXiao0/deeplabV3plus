...
def mobilenet_v2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MobileNetV2:
    ...
    if pretrained:
        preModel = torch.load('/home/chengxiao/pycharmProject/myDeepLabV3Plus/mobilenet_v2-b0353104.pth')
        # paramList = list(preModel.keys())
        # print(paramList)
        modelDict = model.state_dict()
        paramsDict = {}
        for i, [name, NetParam] in enumerate(model.named_parameters()):
            if name in preModel.keys():
                paramsDict[name] = preModel[name]
        modelDict.update(paramsDict)
        model.load_state_dict(modelDict)
    return model

def _initMobileNet(path, model):
...
if __name__ == '__main__':
    model = mobilenet_v2()
    # print(mNet)
    preModel = torch.load('../../mobilenet_v2-b0353104.pth')
    """
    此处需要剔除第一层和最后一层的参数
    """
    paramList = list(preModel.keys())
    ...
    for i, [name, NetParam] in enumerate(model.named_parameters()):
        if name in preModel.keys():
            paramsDict[name] = preModel[name]
    modelDict.update(paramsDict)
    # for i, [name, NetParam] in enumerate(mNet.named_parameters()):
    #     print(name)
    # for i, [name, NetParam] in enumerate(mNet.named_parameters()):
    #     print(name, "|", paramList[i])
    #     print(NetParam.shape, "|", preModel[paramList[i]].shape)
    # for key in preModel.keys():
    #     print(preModel[key].shape)
    # for i, [name, NetParam] in enumerate(mNet.named_parameters()):
    #     paramsDict[name] =preModel[paramList[i]]
    model.load_state_dict(modelDict)
    #
    for i, [name, NetParam] in enumerate(model.named_parameters()):
        print(NetParam)
    ...
    #     print(i)
    #     print(name)
    #     print(NetParam.shape)
    #     print(paramList[i + 1])
    #     print(preModel[paramList[i + 1]].shape)
    #     break
    # _initMobileNet('none', preModel)
    # mNet.cuda()
    # print(mNet)
    # summary(mNet, (3, 244, 244))
